...
public class MainView extends VerticalLayout {
    public MainView(@Autowired PokemonService service) {
        ...
        Button addYourPokemonButton = new Button("Add Your Pokemon", e -> this.addPokemon(service, searchPokemon.getValue(), true));
        Button addRivalPokemonButton = new Button("Add Rival Pokemon", e -> this.addPokemon(service, searchPokemon.getValue(), false));
        addYourPokemonButton.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        addYourPokemonButton.addClickShortcut(Key.ENTER);
        add(searchPokemon, addYourPokemonButton, addRivalPokemonButton);
            ...
            if (isYourPokemon) {
                ...
                ReflectionUtils.doWithFields(weaknesses.damage_relations.getClass(), field -> {
                    ...
                    if (fieldValue.length != 0) {
                        ...
                        for (Type weakType : fieldValue) {
                            types.append(weakType.name).append(", ");
                        }
                        types.append("<br>");
                    }
                });
